...
def insertDB():
    try:
        ...
        print("Insertion des travaux en cours...cela peut prendre un peu de temps")
        #on ajoute les travaux
        read_csv_file_all_travaux(
            ["data/csv/Chauffage.csv", "data/csv/Isolation.csv", "data/csv/Photovoltaique.csv"], 
            ';'
        )
...
def read_csv_file_all_travaux(files, separator):
    # Lecture du fichier CSV csvFile avec le séparateur separator
    # pour chaque ligne, exécution de query en la formatant avec les colonnes columns
    ...
    for csvFile in files:
        if csvFile == "data/csv/Chauffage.csv":
            secondaryColumns = ['energie_chauffage_avt_travaux', 'energie_chauffage_installee', 'generateur', 'type_chaudiere']
            secondaryQuery = "insert into TravauxChauffage values (?, ?, ?, ?, ?)"

        if csvFile == "data/csv/Isolation.csv":
            secondaryColumns = ['poste_isolation', 'isolant', 'epaisseur', 'surface']
            secondaryQuery = "insert into TravauxIsolation values (?,?,?,?,?)"

        if csvFile == "data/csv/Photovoltaique.csv":
            secondaryColumns = ['puissance_installee', 'type_panneaux']
            secondaryQuery = "insert into TravauxPhotovoltaique values (?,?,?)"
        # insertion des travaux
        df = pandas.read_csv(csvFile, sep=separator)
        df = df.where(pandas.notnull(df), 'null')

        columns = ['cout_total_ht', 'cout_induit_ht', 'annee_travaux', 'type_logement', 'annee_construction',  'code_region']
        query = "insert into Travaux values (?, ?, ?, ?, ?, ?, ?)"
        for ix, row in df.iterrows():
            try:
                tab = [id_travaux]
                secondaryTab = [id_travaux]
                ...
                for i in range(len(secondaryColumns)):
                    # pour échapper les noms avec des apostrophes, on remplace dans les chaines les ' par ''
                    if isinstance(row[secondaryColumns[i]], str):
                        row[secondaryColumns[i]] = row[secondaryColumns[i]].replace("'","''")
                    secondaryTab.append(row[secondaryColumns[i]])
                                # Execution des requetes 
                data.commit()
                cursor.execute(query, tuple(tab))
                cursor.execute(secondaryQuery, tuple(secondaryTab))
                id_travaux+=1
            except IntegrityError as err:
                data.rollback()
                print(err)
